...
interface IProps {
  ...
  id: string | number,
  index: number,
  openModal: (id: string | number, title: string)=> void
  getIdToDelete: (id: string | number, title: string)=> void
}
const Todos = ({title, id, index, openModal, getIdToDelete }: IProps) => {
  return (
    <div className="shadow-[#32325d40_0px_2px_5px_-1px,_#0000004d_0px_1px_3px_-1px] rounded-sm px-3 py-5 flex justify-between items-center mt-5 bg-slate-50">
      ...
      <div className="space-x-3 flex ">
        <button onClick={() => openModal(id, title)} className="group relative inline-block text-sm font-medium text-white focus:outline-none focus:ring">
